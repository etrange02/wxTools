...
tools::database::DataBaseResponse* DataBaseOpeningRequest::execute(tools::database::DataBaseManager& dataBaseManager)
{
    ...
    int version = -1;
    if (db->TableExists(TABLE_META_DATA))
    {
        wxSQLite3ResultSet result = db->ExecuteQuery("SELECT " COLUMN_DATA_VALUE " FROM " TABLE_META_DATA " WHERE " COLUMN_DATA_KEY "='" DataBaseVersion "';");
    ...
    {
        wxString request;
        request << "UPDATE " TABLE_META_DATA " SET " COLUMN_DATA_VALUE "='";
        request << newVersion;
...
int DataBaseOpeningRequest::updateTables(wxSQLite3Database& database, int currentVersion)
{
    ...
    if (!database.TableExists(TABLE_META_DATA))
        database.ExecuteUpdate("CREATE TABLE " TABLE_META_DATA "("
                                COLUMN_DATA_KEY " VARCHAR(100),"
                                COLUMN_DATA_VALUE " VARCHAR(100));");

    // We create the row to modify it later.
    database.ExecuteUpdate("INSERT INTO " TABLE_META_DATA " VALUES ('" DataBaseVersion "','0')");
